...
export default function StyleControls({
  ...
  const styleTabSliderProps = {
    trackClassName: 'h-[4px] rounded-full bg-white/[0.06]',
    rangeClassName: 'h-[4px] rounded-full bg-white/[0.18]',
    thumbClassName: 'h-3.5 w-3.5 border border-white/60 bg-[#2a2a31] shadow-[0_0_0_1px_rgba(255,255,255,0.1),0_4px_10px_rgba(0,0,0,0.24)]',
  };

  return (
    <div className="h-full flex flex-col overflow-hidden rounded-[18px] lekha-panel lekha-inspector">
      <div className="shrink-0 border-b border-white/10 px-4 pt-4">
        <p className="lekha-micro-label text-slate-300">Styling</p>
        <div className="mt-5 flex items-end justify-between gap-3">
          {panelTabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => setActivePanelTab(tab.id)}
              className={`relative flex-1 px-1 pb-3 text-center text-[10px] font-bold uppercase tracking-[0.24em] transition-colors ${
                activePanelTab === tab.id ? 'text-white' : 'text-slate-500 hover:text-slate-300'
              }`}
            >
              {tab.label}
              {activePanelTab === tab.id && (
                <span className="absolute inset-x-1 bottom-0 h-px bg-white" />
              )}
            </button>
          ))}
        </div>
    ...
    <div className="flex-1 overflow-y-auto px-4 py-7 custom-scrollbar">
      <h2 className="hidden">
        {selectedTextElement ? 'Text Element Style' : 'Caption Style'}
      </h2>
      ...
      <div className="space-y-10">
        ...
        <div className="space-y-7">
          <div className="flex items-center justify-between">
            <h3 className="text-[9px] font-medium text-slate-400">TYPOGRAPHY</h3>
            <span className="rounded-full border border-white/10 bg-black/30 px-2 py-0.5 text-[9px] font-semibold text-white">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Font Family</Label>
              <span className="rounded bg-[#202020] px-1.5 py-0.5 text-[9px] font-semibold leading-none text-white">
                {scriptLabels[detectedScript] || detectedScript}
              </span>
            </div>
            <Popover open={fontPopoverOpen} onOpenChange={setFontPopoverOpen}>
              <PopoverTrigger asChild>
                <Button
                  variant="outline"
                  role="combobox"
                  className="w-full justify-between rounded-md border-[#30313a] bg-[#17171a] px-3 text-white font-normal transition-all hover:bg-[#1d1d21] hover:text-white active:bg-[#202024] h-9"
                  style={{ fontFamily: getCurrentValue('font_family', 'Inter') }}
                >
                  <span className="mr-2 inline-flex h-5 w-5 shrink-0 items-center justify-center rounded bg-white text-[12px] font-serif italic text-black">N</span>
                  <span className="truncate flex-1 text-left text-[13px] font-semibold">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Font Size</Label>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('font_size', 18)}px</span>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Line Spacing</Label>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('line_spacing', 1.4)}</span>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          {/* Font Style Selector */}
          <div>
            <Label className="mb-1 block text-[9px] font-medium text-slate-400">Font Style</Label>
            <Select
            ...
            >
              <SelectTrigger className="bg-zinc-900 border-white/10 text-white [&>span]:text-white">
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Letter Spacing</Label>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('letter_spacing', 0)}px</span>
                <Button
                  variant="ghost"
                  size="icon"
        ...
        <div className="space-y-7 pt-8 border-t border-white/5">
          <h3 className="text-[9px] font-medium text-slate-400">POSITION</h3>

          {/* Position Y */}
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Position Y</Label>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold tabular-nums text-slate-100">{selectedTextElement ? Math.round(selectedTextElement.customStyle?.top ?? 50) : (captionStyle.position_y ?? 75)}%</span>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          <div>
            <div className="flex items-center justify-between mb-1">
              <Label className="text-[9px] font-medium text-slate-400">Position X</Label>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-semibold tabular-nums text-slate-100">{selectedTextElement ? Math.round(selectedTextElement.customStyle?.left ?? 50) : (captionStyle.position_x ?? 50)}%</span>
                <Button
                  variant="ghost"
                  size="icon"
          ...
          <div className="border-t border-white/5 pt-7">
            <h3 className="text-[9px] font-medium text-slate-400">ALIGNMENT & CASE</h3>
          </div>

          {/* Text Alignment */}
          <div>
            <Label className="mb-1 block text-[9px] font-medium text-slate-400">Text Alignment</Label>
          ...
          {!selectedTextElement && (
            <div>
              <Label className="mb-1 block text-[9px] font-medium text-slate-400">Display Mode</Label>
              <Select
              ...
              >
                <SelectTrigger className="bg-zinc-900 border-white/10 text-white h-9 [&>span]:text-white">
                  <SelectValue />
                </SelectTrigger>
          ...
          {/* Case */}
          <div>
            <Label className="mb-1 block text-[9px] font-medium text-slate-400">Case</Label>
            <div className="bg-zinc-900 border border-white/5 rounded-lg p-1 flex gap-1">
              <button
                onClick={() => {
        ...
        {/* ── 3. COLORS ─────────────────────────────── */}
        <div className="space-y-7 pt-8 border-t border-white/5">
          <h3 className="text-[9px] font-medium text-slate-400">COLORS</h3>

          {/* Text Color */}
          <div>
            <Label className="mb-1 block text-[9px] font-medium text-slate-400">Text Color</Label>
              ...
              <>
                ...
                <div className="flex items-center gap-2 p-2 rounded-lg bg-white/[0.02] border border-white/5">
                  ...
                  <input
                    ...
                    className="w-8 h-8 rounded cursor-pointer bg-transparent"
                  />
                  <span className="text-[10px] font-semibold tabular-nums text-slate-100">Custom</span>
                </div>
              </>
            )}
          </div>
          ...
          {/* Highlight Word */}
          <div>
            <Label className="mb-1 block text-[9px] font-medium text-slate-400">Highlight Word</Label>
        ...
        <div className="space-y-7 pt-8 border-t border-white/5">
          ...
          {/* Background Toggle */}
          <div className="flex items-center justify-between">
            <Label className="text-[9px] font-medium text-slate-400">Background</Label>
            <Switch
          ...
          {(selectedTextElement ? (selectedTextElement.customStyle?.hasBackground !== false) : !!captionStyle.has_background) && (
            <>
              <div>
                <Label className="mb-1 block text-[9px] font-medium text-slate-400">Background Color</Label>
                <input
                  type="color"
              ...
              <div>
                <div className="flex items-center justify-between mb-1">
                  <Label className="text-[9px] font-medium text-slate-400">Background Opacity</Label>
                  <div className="flex items-center gap-2">
                    <span className="text-[10px] font-semibold tabular-nums text-slate-100">{Math.round((getCurrentValue('background_opacity', 0.7)) * 100)}%</span>
                    <Button
                      variant="ghost"
                      size="icon"
              ...
              <div className="space-y-7">
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <Label className="text-[9px] font-medium text-slate-400">Thickness V</Label>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-semibold tabular-nums text-slate-100">{selectedTextElement ? (selectedTextElement.customStyle?.padding ?? 8) : (captionStyle.background_padding ?? 3)}px</span>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => {
                ...
                <div>
                  <div className="flex items-center justify-between mb-1">
                    <Label className="text-[9px] font-medium text-slate-400">Thickness H</Label>
                    <div className="flex items-center gap-2">
                      <span className="text-[10px] font-semibold tabular-nums text-slate-100">{((getCurrentValue('background_h_multiplier', 1.05) - 1) * 100).toFixed(2)}px</span>
                      <Button
                        variant="ghost"
                        size="icon"
                        onClick={() => updateStyle('background_h_multiplier', 1.05)}
                        className="h-4 w-4 text-gray-500 hover:text-white"
                      >
                        <RotateCcw className="w-2.5 h-2.5" />
                      </Button>
                  ...
                  <Slider
                    value={[Math.round(getCurrentValue('background_h_multiplier', 1.05) * 100)]}
        ...
        {/* ── 5. EFFECTS ────────────────────────────── */}
        <div className="space-y-7 pt-8 border-t border-white/5">
          <h3 className="text-[9px] font-medium text-slate-400">EFFECTS</h3>
                    ...
                    {['hollow', 'splice', 'outline'].includes(getCurrentValue('effect_type')) && (
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_thickness', 50)}</span>
                        </div>
                        <Slider value={[getCurrentValue('effect_thickness', 50)]} onValueChange={([val]) => updateStyle('effect_thickness', val, true)} onPointerDown={() => addToHistory && addToHistory()} max={100} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                      </div>
                    )}
                      ...
                      <>
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            ...
                            <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_offset', 50)}</span>
                          </div>
                          <Slider value={[getCurrentValue('effect_offset', 50)]} onValueChange={([val]) => updateStyle('effect_offset', val, true)} onPointerDown={() => addToHistory && addToHistory()} max={100} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                        </div>
                        <div>
                          <div className="flex items-center justify-between mb-1">
                            <Label className="text-xs text-gray-400">Direction</Label>
                            <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_direction', -45)}°</span>
                          </div>
                          <Slider value={[getCurrentValue('effect_direction', -45)]} onValueChange={([val]) => updateStyle('effect_direction', val, true)} onPointerDown={() => addToHistory && addToHistory()} min={-180} max={180} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                        </div>
                      </>
                    )}
                    {['shadow', 'neon', 'lift'].includes(getCurrentValue('effect_type')) && (
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_blur', 50)}</span>
                        </div>
                        <Slider value={[getCurrentValue('effect_blur', 50)]} onValueChange={([val]) => updateStyle('effect_blur', val, true)} onPointerDown={() => addToHistory && addToHistory()} max={100} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                      </div>
                    )}
                    {['shadow', 'echo'].includes(getCurrentValue('effect_type')) && (
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_transparency', 40)}</span>
                        </div>
                        <Slider value={[getCurrentValue('effect_transparency', 40)]} onValueChange={([val]) => updateStyle('effect_transparency', val, true)} onPointerDown={() => addToHistory && addToHistory()} max={100} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                      </div>
                    )}
                    {['neon'].includes(getCurrentValue('effect_type')) && (
                      <div>
                        <div className="flex items-center justify-between mb-1">
                          ...
                          <span className="text-[10px] font-semibold tabular-nums text-slate-100">{getCurrentValue('effect_intensity', 50)}</span>
                        </div>
                        <Slider value={[getCurrentValue('effect_intensity', 50)]} onValueChange={([val]) => updateStyle('effect_intensity', val, true)} onPointerDown={() => addToHistory && addToHistory()} max={100} step={1} className="cursor-pointer" {...styleTabSliderProps} />
                      </div>
                    )}
