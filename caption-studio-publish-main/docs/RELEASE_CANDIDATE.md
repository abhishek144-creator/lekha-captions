...
3. Build fresh artifacts with production API/auth/App Check/legal configuration. Run dependency, privacy, operational and launch-evidence gates. Record results against the exact SHA. Apply the owner's current essential-only scope in `CONTROLLED_LAUNCH_SCOPE.md`; pentest/CA engagements and deferred enterprise/mass-traffic work are not controlled-launch blockers.
