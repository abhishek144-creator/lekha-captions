...
if (scripts["test:browser"]?.includes("playwright test")) pass("cross-browser quality script is registered")
else fail("cross-browser quality script is registered", "Missing Playwright browser test script")

if (scripts["launch:verify-deployed"]?.includes("check-deployed-release.mjs")) pass("deployed release verification is registered")
else fail("deployed release verification is registered", "Missing deployed release verification script")
...
assertFile("Playwright configuration exists", "playwright.config.js")
assertFile("public launch browser suite exists", "e2e/public-launch.spec.js")
assertFile("deployed release verifier exists", "scripts/check-deployed-release.mjs")
assertFile("real-media capacity runner exists", "scripts/media_load_smoke.py")
assertFile("mass launch runbook exists", "docs/MASS_LAUNCH_RUNBOOK.md")
assertFile("production release record template exists", "docs/RELEASE_RECORD_TEMPLATE.md")
assertFile("external-style security review exists", "docs/EXTERNAL_STYLE_SECURITY_REVIEW_2026-09-02.md")
assertFile("legal and tax readiness review exists", "docs/LEGAL_TAX_READINESS_REVIEW_2026-09-02.md")
assertFile("hashed backend dependency lock exists", "backend/requirements.lock")
assertFile("marketing refund policy exists", "landing-next/app/refund/page.js")
assertFile("marketing acceptable use policy exists", "landing-next/app/acceptable-use/page.js")

const playwrightConfig = readText("playwright.config.js")
assertIncludes("browser suite can target a deployed origin", playwrightConfig, "PLAYWRIGHT_BASE_URL")
const browserSuite = readText("e2e/public-launch.spec.js")
assertIncludes("browser suite waits for rendered UI instead of network silence", browserSuite, "waitUntil: 'domcontentloaded'")
const deployedVerifier = readText("scripts/check-deployed-release.mjs")
assertIncludes("deployed verification probes the editor route", deployedVerifier, "/Dashboard?entry=editor")
assertIncludes("deployed verification rejects cross-origin redirects", deployedVerifier, "frontendResponse.finalUrl")
...
assertIncludes("production frontend requires legal business identity", viteConfig, "VITE_LEGAL_BUSINESS_NAME")
assertIncludes("production frontend requires a grievance officer", viteConfig, "VITE_GRIEVANCE_OFFICER_NAME")
assertIncludes("frontend embeds an immutable release identity", viteConfig, "lekha-release")
assertExcludes("frontend CSP does not allow arbitrary HTTPS connections", viteConfig, "connect-src 'self' https:;")
...
assertIncludes("GitHub Actions runs Python static security analysis", workflow, "bandit -r backend")
assertIncludes("GitHub Actions scopes the Chromium workaround to CI", workflow, 'PUPPETEER_CI_NO_SANDBOX: "1"')
assertIncludes("GitHub Actions provides Firebase App Check build configuration", workflow, "VITE_FIREBASE_APP_CHECK_SITE_KEY")
...
const syntheticMonitor = readText("scripts/check-synthetic-health.mjs")
assertIncludes("synthetic monitoring checks Google validating DNS", syntheticMonitor, "https://dns.google/resolve")
...
assertIncludes("marketing footer links the refund policy", marketingFooter, 'href="/refund/"')
...
assertIncludes("production requires the configured GCS signing identity", backendMain, 'required_production_settings["GCS_SIGNING_SERVICE_ACCOUNT"]')
...
const terraformMain = readText("deploy/gcp/terraform/main.tf")
const terraformVariables = readText("deploy/gcp/terraform/variables.tf")
const runtimeEnvExample = readText("deploy/gcp/runtime.env.example")
const directUploadApi = readText("backend/direct_upload_api.py")
const maintenanceApi = readText("backend/api/maintenance.py")
const mediaScanJobs = readText("backend/services/media_scan_jobs.py")
const schedulerAuth = readText("backend/services/scheduler_auth.py")
const uploadSecurity = readText("backend/services/upload_security.py")
assertIncludes("upload CORS includes the editor origin", terraformVariables, '"https://app.lekhacaptions.com"')
assertIncludes("runtime CORS includes the editor origin", runtimeEnvExample, "https://app.lekhacaptions.com")
assertIncludes("URL signer reads only export objects", terraformMain, "/objects/exports/")
assertIncludes("URL signing uses a signBlob-only custom role", terraformMain, 'permissions = ["iam.serviceAccounts.signBlob"]')
assertExcludes("URL signing does not grant Token Creator", terraformMain, "roles/iam.serviceAccountTokenCreator")
assertIncludes("direct uploads have an authenticated reload-resume route", directUploadApi, '"/api/uploads/resume"')
assertIncludes("completed direct uploads enqueue eager malware scans", directUploadApi, "enqueue_scan(uid, req.file_id)")
assertIncludes("malware scans use a durable outbox", mediaScanJobs, "media_scan_outbox")
assertIncludes("upload scan-state transitions live in a service", uploadSecurity, "class UploadSecurityService")

const publicClaims = [
  readText("src/pages/Faq.jsx"),
  readText("src/components/landing/FeaturesSection.jsx"),
  readText("src/components/landing/LanguageReachSection.jsx"),
  readText("src/components/dashboard/UploadModal.jsx"),
  readText("landing-next/lib/faq.js"),
  readText("landing-next/lib/site.js"),
].join("\n")
assertExcludes("public claims omit an unmeasured accuracy percentage", publicClaims, "90–98%")
assertExcludes("public claims omit an unsupported universal language count", publicClaims, "115+")
assertExcludes("public claims do not promise exact-millisecond caption timing", publicClaims, "exact millisecond")
...
const sidebarNav = readText("src/components/dashboard/SidebarNav.jsx")
const localDraftStorage = readText("src/lib/localDraftStorage.js")
...
assertIncludes("account logout clears every user-scoped local draft", userAccount, "clearUserLocalDrafts(currentUser?.uid)")
assertIncludes("account deletion clears every user-scoped local draft", userAccount, "clearUserLocalDrafts(currentUser.uid)")
assertIncludes("dashboard logout clears every user-scoped local draft", sidebarNav, "clearUserLocalDrafts(currentUser?.uid)")
assertIncludes("local draft cleanup removes per-project keys", localDraftStorage, "key?.startsWith(prefix)")
assertIncludes("logout cleanup removes pending upload receipts", localDraftStorage, "lekha.pendingDirectUpload.v1")
...
assertExcludes("API readiness does not require warm export workers", backendMain, 'checks["export_worker"]')
assertExcludes("API readiness does not require warm transcription workers", backendMain, 'checks["transcription_worker"]')
assertIncludes("scale-to-zero capacity remains observable", backendMain, '"idle_scaled_to_zero"')
assertIncludes("transcription requests enqueue immediately", backendMain, "transcription_immediate_dispatch_failed")
assertIncludes("scheduler can recover transcription outbox delivery", maintenanceApi, '"/transcription-dispatch"')
assertIncludes("scheduler can recover media-scan outbox delivery", maintenanceApi, '"/media-scan-dispatch"')
assertIncludes("production proxy uploads are disabled", backendMain, "Production uploads must use the direct resumable storage flow")
assertIncludes("exports require security-clean source media", backendMain, "_assert_upload_clean(req.file_id)")
assertIncludes("exports reuse upload-time media identity", backendMain, "_source_media_identity(req.file_id, input_path)")
assertIncludes("render cache survives worker replacement", backendMain, "download_render_cache")
assertIncludes("render cache is persisted to durable storage", backendMain, "upload_render_cache")
assertIncludes("animated template renders can use the exact-request cache", backendMain, "cache_eligible = render_cache_eligible")
assertIncludes("Cloud Scheduler verifies Google OIDC tokens", schedulerAuth, "verify_oauth2_token")
assertIncludes("maintenance endpoints are extracted from the application module", backendMain, "create_maintenance_router")
assertIncludes("render cache lives in the rendering package", backendMain, ".rendering.cache")
assertIncludes("malware scanning lives in the services package", backendMain, ".services.malware")
assertIncludes("payment reconciliation has a scheduler endpoint", maintenanceApi, '"/payment-reconciliation"')
assertIncludes("Firebase Hosting serves the built SPA", readText("firebase.json"), '"public": "dist"')
assertIncludes("Cloud Run API candidate is infrastructure-managed", terraformMain, 'google_cloud_run_v2_service" "api')
assertIncludes("Cloud Run reaches private Redis through VPC access", terraformMain, 'google_vpc_access_connector" "api')
assertIncludes("Cloud Run keeps malware scanning beside the API", terraformMain, 'name  = "clamav"')
assertIncludes("maintenance jobs are externalized to Cloud Scheduler", terraformMain, 'google_cloud_scheduler_job" "maintenance')
assertIncludes("render workers use the benchmark profile variable", terraformMain, "var.render_worker_machine_type")
assertIncludes("Spot overflow workers are bounded", terraformMain, 'google_compute_instance_template" "worker_spot')
assertIncludes("Spot overflow capacity can scale to zero", terraformMain, 'resource "google_compute_region_autoscaler" "worker_spot"')
assertIncludes("Spot overflow starts at zero", terraformMain, "min_replicas    = 0")
assertIncludes("baseline render capacity keeps three warm workers", terraformVariables.replace(/\r\n/g, "\n"), 'variable "render_worker_min_replicas" {\n  type        = number\n  description = "Minimum on-demand render workers"\n  default     = 3')
assertIncludes("Cloud Run API defaults to scale-to-zero", terraformVariables.replace(/\r\n/g, "\n"), 'variable "cloud_run_api_min_instances" {\n  type    = number\n  default = 0')
assertIncludes("Cloud Run API is enabled by the production infrastructure profile", terraformVariables.replace(/\r\n/g, "\n"), 'variable "enable_cloud_run_api" {\n  type        = bool\n  description = "Create the parallel Cloud Run API candidate and scheduler jobs; traffic cutover remains an explicit load-balancer operation"\n  default     = true')
assertIncludes("transcription has an independent MIG", terraformMain, 'google_compute_region_instance_group_manager" "transcription')
assertExcludes("transcription image has no staging queue baked in", readText("Dockerfile"), "caption_transcription_jobs_staging")
...
const errorBoundary = readText("src/components/ErrorBoundary.jsx")
const frontendMonitoring = readText("src/lib/frontendMonitoring.js")
...
assertIncludes("frontend crashes are reported to monitoring", errorBoundary, "captureFrontendException")
assertIncludes("frontend monitoring sends no personal data by default", frontendMonitoring, "sendDefaultPii: false")
assertIncludes("customer crash page provides a support reference", errorBoundary, "Support reference:")
assertExcludes("customer crash page does not expose raw error messages", errorBoundary, "this.state.error.toString()")
assertExcludes("customer crash page does not expose React component stacks", errorBoundary, "this.state.errorInfo.componentStack")
...
assertIncludes("privacy policy names a grievance process", privacyPolicy, "Grievances and complaints")
assertIncludes("consumer complaints have a published response target", terms, "within 48 hours")
...
assertIncludes("export renderer can reuse a warm browser", renderer, "connectWarmBrowser(puppeteer)")
assertIncludes("export renderer skips remote fetches for packaged fonts", renderer, "needsRemoteRenderFont")
...
assertIncludes("container installs repository render fonts for Chromium", dockerfile, "/usr/local/share/fonts/truetype/lekha")
assertIncludes("container refreshes the system font cache", dockerfile, "fc-cache -f")
assertIncludes("container builds the complete template font bundle", dockerfile, "build_render_font_bundle.mjs")
assertIncludes("renderer verifies and injects bundled fonts", renderer, "bundled font digest mismatch")
